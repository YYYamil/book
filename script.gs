function doPost(e) {
  try {
    ...
    if (data.action === "crearReserva") {
      ...
      Logger.log(`Procesando reserva, pernocta: ${pernoctaValue}, raw: ${data.pernocta}`);
      ...
      // Actualizar disponibilidad solo si pernocta es true
      if (data.pernocta === "true") {
        ...
        if (filaEncontrada) {
          nuevosOcupados = parseInt(filaEncontrada[2]) + cantidad; // Increment existing occupied count
